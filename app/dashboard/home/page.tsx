...
import { TrendingUp, TrendingDown, UserRound, Coins, Gift, Tag } from "lucide-react";
...
function fmt(n:number) { return n>=10000?`${(n/1000).toFixed(1).replace(".0","")}k`:n.toLocaleString("fr-FR"); }

// ─── Chart configs ────────────────────────────────────────────────────────────
// Couleurs explicites (identiques à la preview). On n'utilise pas hsl(var(--chart-x))
// car --chart-x est défini en oklch dans globals.css → hsl(oklch(...)) est invalide.
const C1 = "#3a64c4"; // bleu
...
const cfgRewards: ChartConfig = {
  cadeaux: { label:"Cadeaux récupérés", color:C1 },
};
...
function Delta({ value, withLabel = true }: { value: number; withLabel?: boolean }) {
  const pos = value >= 0;
  return (
    <span className={cn("flex items-center gap-1 font-medium", pos ? "text-emerald-600" : "text-red-500")}>
      ...
      {pos ? "+" : ""}{value.toFixed(1)}%{withLabel ? " vs période précédente" : ""}
    </span>
  );
}
...
function StatCard({
  icon: Ico, label, value, delta,
}: {
  icon: typeof UserRound; label: string; value: number; delta: number;
}) {
  const pos = delta >= 0;
  return (
    <Card className='p-0'>
...
export default function HomePage() {
  ...
  return (
    <div className="space-y-6 pb-2">
      ...
      {/* KPI — statistiques grossières */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard icon={UserRound} label="Nouveaux clients"     value={kpiC}  delta={d.delta.c}  />
        <StatCard icon={Coins}     label="Points attribués"     value={kpiP}  delta={d.delta.p}  />
        <StatCard icon={Gift}      label="Cadeaux récupérés"    value={kpiR}  delta={d.delta.r}  />
        <StatCard icon={Tag}       label="Promotions utilisées" value={kpiPr} delta={d.delta.pr} />
      </div>
